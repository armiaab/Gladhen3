...
// Returns true if the UTF-8 byte needs percent-encoding.
// Only RFC 3986 unreserved characters pass through unencoded.
...
static void BuildGladhenUri(IShellItemArray* psia, WCHAR* uri, size_t uriSize) noexcept
{
    ...
    for (DWORD i = 0; i < count; i++)
    {
        ...
        if (SUCCEEDED(psi->GetDisplayName(SIGDN_FILESYSPATH, &path)) && path)
        {
            ...
            // Convert to UTF-8 so non-ASCII filenames encode correctly.
...
{
    LONG         _ref;
    const CLSID& _clsid;   // references the static CLSID; no lifetime concern
